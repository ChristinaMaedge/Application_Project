...
Insgesamt wird Vorgehen wird in mehreren Stufen untergliedert: Zunächst werden im Rahmen einer Korrelationsanalyse Zusammenhänge zwischen dem Umsatz und potentiell beeinflussenden Variablen untersucht, um zu prüfen, welche Variablen für die Aufnahme in ein Regressionsmodell geeignet erscheinen. Da einfache statistische Maßzahlen zur Beschreibung der Daten nicht immer ausreichen und ein und demselben Wert des Korrelationskoeffizienten unterschiedliche Zusammenhänge (neben dem linearen) zugrunde liegen können, werden für die potentiell beeinflussenden Variablen in einem weiteren Schritt Streudiagramme erstellt (vgl. hierzu auch Anscombes Quartett aus dem Jahr 1973). Im Anschluss können die Modelle auf einer vergleichsweise fundierten / begründeten Basis erstellt werden. 

## 6.2 Zusammenhangsanalysen
...
## 6.3 Visualisierung der Zusammenhänge zwischen ausgewählten Variablen 

**Betrachtung der Warengruppe 1**
...
Zunächst wird der isolierte Einfluss der Variable `Temperatur` überprüft:
```{r}
#library(ggplot2)
#library(dplyr)
lm_train_WG1_e1 <- lm(Umsatz ~ Temperatur, data = df_train) # Benennung Lineares Modell, Training, Warengruppe, einfaches Modell 1
#tidy(lm_train_WG1_e1)
#glance(lm_train_WG1_e1)
summary.lm(lm_train_WG1_e1)
```
(Bewertung/Interpretation)

Als nächstes werden Modelle für die verschiedenen `Sommerferien`-Variablen betrachtet:
```{r}
# Einfaches Modell Sommerferien Schleswig-Holstein:
lm_train_WG1_e2 <- lm(Umsatz ~ SommerferienSH, data = df_train)
summary.lm(lm_train_WG1_e2)

# Einfaches Modell Sommerferien Nordrhein-Westfalen:
lm_train_WG1_e3 <- lm(Umsatz ~ SommerferienNRW, data = df_train)
summary.lm(lm_train_WG1_e3)

# Einfaches Modell Sommerferien Niedersachsen:
lm_train_WG1_e4 <- lm(Umsatz ~ SommerferienNDS, data = df_train)
summary.lm(lm_train_WG1_e4)

# Einfaches Modell Sommerferien Hessen:
lm_train_WG1_e5 <- lm(Umsatz ~ SommerferienNDS, data = df_train)
summary.lm(lm_train_WG1_e5)
```

(Bewertung/Interpretation)

Ein weiteres einfaches Modell wird für die Variable `Sommer` erstellt:
```{r}
lm_train_WG1_e6 <- lm(Umsatz ~ Sommer, data = df_train)
summary.lm(lm_train_WG1_e6)
```

Verglichen wird der Einfluss der `Sommer`-Variable mit der Variable `Jahreszeit`:
```{r}
lm_train_WG1_e7 <- lm(Umsatz ~ Jahreszeit, data = df_train)
summary.lm(lm_train_WG1_e7)
```


Abschließend für die einfachen Modelle wird der isolierte Einfluss der Variable `Silvester` geprüft:

```{r}
lm_train_WG1_e7 <- lm(Umsatz ~ Silvester, data = df_train)
summary.lm(lm_train_WG1_e7)
```
