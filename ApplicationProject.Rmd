...
```{r}
lm1_predict <- predict(lm1_train, newdata = df_lm_test_WG1_28)
```
...
```{r}
caret::postResample(pred = lm1_predict, obs = df_lm_test_WG1_28$Umsatz)
```
...
Wir können jetzt die beste Teilmengenauswahl für den gesamten Datensatz durchführen, um zum einen das 30-Variblen-Modell zu erhalten.
Dieses Modell wird mit dem 29- und 36-Variablen-Modell verglichen.
...
```{r}
lm3_predict <- predict(lm3_train, newdata = df_lm_test_WG2_30)
```

Als Ergebnis wird ein Vektor erzeugt, der für jede Beobachtung des Test-Samples den geschätzten (vorhergesagten) Umsatzwert speichert. Die Gütewerte lassen sich mit `caret::postResample` ausgeben:
```{r}
caret::postResample(pred = lm3_predict, obs = df_lm_test_WG2_30$Umsatz)
lm3_predict

df_lm_test_WG2_30 <- df_lm_test_WG2_30 %>%
  mutate(predicted = lm3_predict)
```
