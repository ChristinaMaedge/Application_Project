...
Wir wollen nun die ermittelten Güte-Kennzahlen für die verschiedenen naiven Modelle zusammen bringen. Dafür erstellen wir eine Vergleichstabelle (prog_naiv_vgl_kennz), die die Kennzahlen je Modell für 2018 enthält. Im ersten Schritt für alle Warengruppen und Wochentage.
...
### 6.2.3 Eliminierung von Multikollinearität

**Erstellung eines ersten Regressionsmodells**

Das folgende Regressionsmodell enthält alle unabhängigen Variablen:
...
Zum einen kann die Multikollinearität für einen gegebenen Prädiktor (p) bewertet werden, indem ein Score berechnet wird, der als Varianzinflationsfaktor (oder VIF) bezeichnet wird und misst, wie stark die Varianz eines Regressionskoeffizienten aufgrund der Multikollinearität im Modell aufgeblasen wird; zum anderen durch die Korrelation der Variablen untereinander
...
Da die Einflussvariablen mitunter stark korrelieren und die Grenze von .7 teilweise fast ankratzen, muss in Erwägung gezogen werden, auch einge der stark miteinander korrelierten Variablen zu eliminieren, da bspw. die schrittweise Regression bei Multikollinearität versagt.
...
Man kann sehen, dass das Entfernen der beiden Variablen `Datum` und `Jahr` die Modellleistungsmetriken nicht sehr beeinflusst. Mit anderen Worten, die moellgenauigkeit leidet nicht unter dem Entfernen der beiden Variablen.


## 6.3 Lineare Regressionsmodelle 
...
**Schritt 1**: Lasse $M_0$ das Nullmodell bezeichnen, das keine Prädiktoren enthält. Dieses Modell sagt einfach den Stichprobenmittelwert für jede Beobachtung voraus.
...
```{r}
forward <- regsubsets(Umsatz ~ ., df_lm_train_WG1, nvmax = 20, method = "forward")
```
...
```{r}
backward <- regsubsets(Umsatz ~ ., df_lm_train_WG1, nvmax = 20, method = "backward")
```
...
## [1] 20
```
Es ist erkennbar, dass die Ergebnisse keine bzw. nur marginal unterschiedliche Modelle identifizieren, die als die besten angesehen werden. Die ajustierte $R^2$-Statistik legt nahe, dass das 20-Variablen-Modell bevorzugt wird, genau wie der $C_p$. Die $BIC$-Statistik schlägt das 19-Variablen-Modell vor.

Auch wenn die Ergebnisse nur schwach voneinander abweichen, können die Variablen und Koeffizienten, die diese Modelle enthalten, mit der `coef`-Funktion verglichen werden:

```{r}
coef(best_subset_WG1, 20)

coef(best_subset_WG1, 20)
```

Der gleiche Prozess kann durch schrittweise Vorwärts- und Rückwärtsauswahl durchgeführt werden und noch mehr Optionen für optimale Modelle liefern:
```{r}
forward <- regsubsets(Umsatz ~ ., df_lm_train_WG1, nvmax = 20, method = "forward")
backward <- regsubsets(Umsatz ~ ., df_lm_train_WG1, nvmax = 20, method = "backward")
...
Wenn das optimale $C_p$ für vorwärts und rückwärts schrittweise bewertet wird, ist erkennbar, wir, dass beide vorschlagen, dass ein 20-Variablen-Modell die $C_p$-Statistik minimiert, ähnlich dem oben beschriebenen besten Teilmengenansatz.


### Direkte Schätzung des Testfehlers

Nun wird der Validierungssatzfehler für das beste Modell jeder Modellgröße berechnet. Zuerst wird eine Modellmatrix aus den Testdaten erstellt. Die Funktion `model.matrix` wird in vielen Regressionspaketen zum Erstellen einer X-Matrix aus Daten verwendet.
...
validation_errors <- vector("double", length = 20)
...
Es ist erkennbar, dass eher das 17-Variablen-Modell, das durch den besten Teilmengenansatz erzeugt wird, den niedrigsten Test-MSE erzeugt. 



# 7 Anwendung von Machine Learning Verfahren
