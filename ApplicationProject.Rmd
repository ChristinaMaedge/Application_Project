...
## 3.2 Vereinigung der Datensätze
...
```{r}
df <- left_join(df, KiWo, by = "Datum")
df <- left_join(df, Wetter, by = "Datum")
head(df)
```

## 3.3 Korrektur der Anzahl Nachkommastellen für einzelne Variablen
...
```{r}
df <- df %>% arrange(Datum, Warengruppe)
...
!!! Einschub: Nur temporär, später entfernen !!!
```{r}
...
Wir wollen nun einige naive Modelle einsetzen, um die Umsätze je Warengruppe zu prognostizieren. Wir arbeiten dafür mit dem vollständigen Datensatz `df_voll`, der für jeden Tag und jede Warengruppe eine Zeile enthält, fehlenden Daten sind mit `NA` gefüllt.

## 5.1 Prognose der Umsätze aufhand des Vorwochenwertes

!!! WEITERES VORGEHEN:
- df_voll aufblähen zu Analysedatensatz:
* enthält alle Datümer und Warengruppen, fehlende Daten mit NA
* fülle systematisch die Lücken, damit wir bspw. Prognose auf Basis des Vorwochenwertes machen können, nehme dafür ggf. die Vorjahreswerte, falls vorhanden
* Vorsicht: Bspw. der 25./26.12. und 1.1. fehlen immer und dafür macht es Sinn, die fehlenden Werte aus Novemberwerten zu ersetzen und nicht mit Vorwochenwert. Sonst bekommt man bspw. für den 8.1. einen zu hohen Schätzer

ich kümmere mich um Kap 5, Christina um Kap 6, Kap 1-4 möglichst in Ruhe lassen

AUSTAUSCH mit Christina am Freitag späten Vormittag




Wir ergänzen den Vorwochenwert (Umsatz_lag7):

```{r}
# lag7 ergänzen
df_voll <- df_voll %>% mutate(Umsatz_lag7 = lag(Umsatz, n=7))
