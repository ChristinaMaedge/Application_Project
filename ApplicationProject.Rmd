...
In dieser Projektarbeit wird ein Prognosemodell entwickelt. Wir wollen damit für eine Bäckereifiliale den **täglichen Umsatz pro Warengruppe** schätzen.
...
Um die beste Teilmengenauswahl durchzuführen, passen wir für jede mögliche Kombination der p-Prädiktoren eine separate Regression der kleinsten Quadrate an. Das heißt, wir passen alle p-Modelle an, die genau einen Prädiktor enthalten, alle $(p_2) = p (p - 1) / 2$-Modelle, die genau zwei Prädiktoren enthalten, und so weiter. Wir betrachten dann alle resultierenden Modelle mit dem Ziel, das beste zu identifizieren.
...
* Passe alle ($p_k$) Modelle an, die genau $k$ Prädiktoren enthalten.
* Wähle die besten unter diesen ($p_k$) Modelle, und nenne es $M_k$. Hier wird "beste Modelle" in der Form definiert, dass diese die kleinsten RSS oder äquivalent die größten $R^2$ haben.
...
* Wähle das beste unter diesen $p - k$-Modellen aus und nenne es $M_\(k_+_1)$. Hier wird das beste Modell als das mit dem kleinsten $RSS$ oder dem höchstes $R^2$ definiert.
...
![](Ergebnisvergleich_Lineare_Modelle.png)
...
$minimiere(SSE = \sum_{i \epsilon R_1}(y_i-c_1)^2+\sum_{i \epsilon R_2}(y_i-c_2)^2$
