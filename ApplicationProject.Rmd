...
Wir wollen jetzt die Ergebnisse der verschiedenen naiven Modelle vergleichen. Wir haben in diesem Kapitel die folgenden Analysedatensätze verwendet und gefüllt:
...
Nun vereinen wir die relativen Abweichungen in einem Datensatz zusammen, je Datum, Warengruppe und Modell, um damit Facetten-Plots der Dichteverteilung, Boxplots und Violinplots der Verteilungen zu erstellen.

Als Grundgerüst (Datum, Warengruppe) für die gemeinsame Tabelle dient uns der ursprüngliche Datensatz `df_naiv`. Wir starten ab dem 01.08.2013, weil wir ab diesem Datum Schätzer für alle Modelle haben. Und wir streichen die Datensätze, für die die Umsätze in den Rohdaten fehlen.

```{r}
prog_naiv_vgl_relAbw <- df_naiv %>%
  filter(Umsatz_NA == FALSE) %>% # fehlende Umsätze in den Rohdaten raus nehmen
  filter(Datum >= "2013-08-01") %>%  # starte ab 01.08.2013
  select(Datum, Warengruppe) # behalte nur die Spalten Datum und Warengruppe
```
...
```{r}
prog_naiv_vgl_relAbw <- prog_naiv_vgl_relAbw %>% 
  pivot_longer(cols=-c("Datum", "Warengruppe"), names_to="Modell", values_to="Abweichung_rel")
```

Stelle nun die Verteilung der relativen Abweichung für die verschiedenen naiven Modelle in einem Plot dar, je Warengruppe:

```{r}
prog_naiv_vgl_relAbw %>%
  filter(Warengruppe==1) %>%
  ggplot(data=prog_naiv_vgl_relAbw, mapping=aes(x=Abweichung_rel*100)) +
  geom_density(aes(fill=Modell), alpha=0.3) + 
  scale_fill_brewer(palette = "Paired") +
  ggtitle("WG1 - Vergleich der naiven Modelle: Relative Abweichung") +
  xlab("rel. Abweichung (%)") + 
  ylab("Dichte")
```

!!! auch für die übrigen Warengruppen
!!! Alternative: Boxplots / Violinplots

Danach bringen wir die ermittelten Güte-Kennzahlen in einer weiteren Tabelle zusammen. 
