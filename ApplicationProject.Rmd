...
prog_naiv_vgl_relAbw <- df_naiv %>%
  filter(Umsatz_NA == FALSE) %>% # fehlende Umsätze in den Rohdaten raus nehmen
  filter(Datum >= "2013-08-01") %>%  # starte ab 01.08.2013
  select(Datum, Warengruppe, Jahr, Wochentag_c) # behalte nur diese Spalten
```
...
```{r}
prog_naiv_vgl_relAbw <- prog_naiv_vgl_relAbw %>% 
  pivot_longer(cols=-c("Datum", "Warengruppe", "Jahr", "Wochentag_c"), names_to="Modell", values_to="Abweichung_rel")
```
...
!!! UND: Facet_wrap, für ein Modell die Dichteverteilung für die Wochentage nebeneinander und die verschiedenen Modelle untereinander, wenn möglich
(siehe R Tutorial)
...
- Detailanalyse der Tage mit den Top10 Abweichungen nach oben und nach unten:
Einschränken auf 2018 (damit wir das später mit den anderen Modellen vergleichen können!), Vergleichstabelle für die verschiedenen Modelle. Gibt es Tage, die in allen Modellen schlecht prognostiziert werden? Für die zu niedrigen Prognosen war das vor allem der Silvester in WG5 (Kuchen = Berliner). Aber was ist mit den Tagen, an denen die Prognose zu hoch war? Den 7. Januar haben wir für einige Modelle schon erklärt. Weitere Tage?
