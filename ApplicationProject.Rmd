...
# 3 Datenaufbereitung und Erstellung von Rohdatensatz sowie Analysedatensätzen
...
Zunächst wird eine Variable für alle Jahreszeiten erstellt bzw. eingelese und an den bestehenden Rohdatensatz hinzugefügt:
```{r}
...
Sinn und Zweck kann es allerdings nicht sein, dass die Prognosen eines Umsatzes am 03.01. sich an den Umstätzen des 24.12. orientieren. Folglich bedarf es eines weiteren Analysedatensatzes, der eine vollständige Zeitreihe enthält. Wie die aufgrund der vorliegenden Datenbasis entstehenden unvollständigen Zeilen mit sinnvollen Werten aufzufüllen sind, ist noch zu klären.

Folgend wird zunächst ein Analysedatensatz `AS1` erstellt, der eine komplette Zeitreihe enthält. Dabei wird in Kauf genommen, dass dieser zunächst viele fehlende Werte enthalten wird, die im weiteren Verlauf für die einzelnen Modelle sinnvoll zu ergänzen sind:

```{r}
## Hinzufügen der vollständigen Zeitreihe
Zeitreihe_voll <- read_csv2("data/Zeitreihe_voll.csv")
AS1 <- left_join(Zeitreihe_voll, df, by = "Datum")
