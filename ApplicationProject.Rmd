...
library(broom)
glance(lm_WG1_28_train)
```

Dann wird (die Modellgüte) anhand der *Test*-Stichprobe getestet:

```{r}
lm_WG1_28_predict <- predict(lm_WG1_28_train, newdata = df_lm_test_WG1_28)
```
...
```{r}
caret::postResample(pred = lm_WG1_28_predict, obs = df_lm_test_WG1_28$Umsatz)
```
...
```{r}
lm_WG1_36_train <- lm(Umsatz ~ ., data = df_lm_train_WG1)
glance(lm_WG1_36_train)
```

```{r}
lm_WG1_36_predict <- predict(lm_WG1_36_train, newdata = df_lm_test_WG1)
```

Als Ergebnis wird ein Vektor erzeugt, der für jede Beobachtung des Test-Samples den geschätzten (vorhergesagten) Umsatzwert speichert. Die Gütewerte lassen sich mit `caret::postResample` ausgeben:
```{r}
caret::postResample(pred = lm_WG1_36_predict, obs = df_lm_test_WG1$Umsatz)
```
...
```{r}
best_subset_WG2 <- regsubsets(Umsatz ~ ., df_lm_train_WG2, nvmax = 36)
```
...
Es ist erkennbar, dass die Ergebnisse leicht unterschiedliche Modelle identifizieren, die als die besten angesehen werden. Die ajustierte $R^2$-Statistik legt nahe, dass das 32-Variablen-Modell bevorzugt wird, die $BIC$-Statistik schlägt das 19-Variablenmodell vor und der $C_p$ das 27-Variablen-Modell vor.
