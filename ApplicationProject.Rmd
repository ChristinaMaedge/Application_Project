...
## 1.2 Zielsetzung 
Wir testen verschiedene einfache und komplexe Modelle und bedienen uns dabei an Techniken aus dem Bereich Machine Learning und Deep Learning. Möglicherweise finden wir ein Verfahren, dass den anderen überlegen ist und die beste Prognosegüte für alle Warengruppen aufweist. 

Oder es stellt sich heraus, dass das Verhalten der Warengruppen zu unterschiedlich ist und wir verschiedene Techniken für verschiedene Warengruppen benötigen.

In jedem Fall wird ein Prognosemodell entworfen, das Bäckereien eine bessere Planungsgrundlage auf Warengruppenebene bietet.
...
```{r}
best_subset_WG1 <- regsubsets(Umsatz ~ ., df_lm_train_WG1, nvmax = 39)
```
...
Für ein Modell mit einer Variablen kann beobachtet werden, dass die erzeugte Dummy-Variable `Wochentag_cSonntag` ein Sternchen hat, was signalisiert, dass ein Regressionsmodell mit Umsatz ~ Wochentag das beste Einzelvariablenmodell ist.  Das beste 2-Variablen-Modell ist `Umsatz` ~ `Wochentag_cSonntag` + `SommerferienSH`. Das beste 3-Variablen-Modell ist `Umsatz` ~ `Wochentag_cSonntag` + `Ostern` + `Ostern_ext`. Und so weiter.
...
```{r}
forward <- regsubsets(Umsatz ~ ., df_lm_train_WG1, nvmax = 39, method = "forward")
```
...
```{r}
backward <- regsubsets(Umsatz ~ ., df_lm_train_WG1, nvmax = 39, method = "backward")
```
...
Es ist erkennbar, dass die Ergebnisse leicht unterschiedliche Modelle identifizieren, die als die besten angesehen werden. Die ajustierte $R^2$-Statistik legt nahe, dass das 30-Variablen-Modell bevorzugt wird, die $BIC$-Statistik schlägt das 22-Variablenmodell vor und der $C_p$ das 27-Variablen-Modell vor.

Die Variablen und Koeffizienten, die diese Modelle enthalten, können mittels der `coef`-Funktion verglichen werden:
```{r}
# 22-Variablen-Modell
coef(best_subset_WG1, 22)
```
...
![](LM_ModellVergleichWG1_27VariablenModell.png)

Beide Modelle werden im weiteren Verlauf erstellt und miteinander verglichen.
...
Vergleicht man die Koeffizienten wiederum mit 30-Variablen-Modell der best subset selection, kommt man zu dem Schluss, dass auch hier die meisten ausgewählten Prädiktoren (28) übereinstimmen, aber dennoch kleine Abweichungen bestehen:

![](LM_ModellVergleichWG1_30_31VariablenModell.png)
...
validation_errors <- vector("double", length = 39)
...
Es ist erkennbar, dass ein 28-Variablen-Modell, das durch den besten Teilmengenansatz erzeugt wird, den niedrigsten Test-MSE erzeugt. 
...
```{r}
final_best27_bss <- regsubsets(Umsatz ~ ., data = df_lm_train_WG1, nvmax = 39)
coef(final_best27_bss, 27)
```
...
```{r}
lm_WG1_27_bss_train <- lm(Umsatz ~ Datum + Jahr + KielerWoche + Bewoelkung + Temperatur + Donnerstag + Freitag + Montag + Samstag + Sonntag + Februar +	Januar + Oktober + September + SommerferienSH + SommerferienNRW + SommerferienNDS + Feiertag + Ostern + ChristiHimmelfahrt + Pfingsten + TDE + Ostern_ext + ChristiHimmelfahrt_ext + Pfingsten_ext + Herbst +	Sommer, data = df_lm_train_WG1_27)

library(broom)
glance(lm_WG1_27_bss_train)
```

Dann wird (die Modellgüte) anhand der *Test*-Stichprobe getestet:

```{r}
lm_WG1_27_bss_predict <- predict(lm_WG1_27_bss_train, newdata = df_lm_test_WG1_27)
```
...
```{r}
caret::postResample(pred = lm_WG1_27_bss_predict, obs = df_lm_test_WG1_27$Umsatz)
```
...
df_lm_test_WG1_27 <- df_lm_test_WG1_27 %>%
  mutate(predicted = lm_WG1_27_bss_predict)
...
# füge das Modell hinzu
temp <- temp %>% mutate(Modell = "best27_bss_WG1")
...
**Teilmengenauswahl für das 27-Variablen-Modell gemäß backward stepwise**

Dieses Modell wird nun verglichen mit dem 27-Variablen-Modell gemäß backward stepwise:
```{r}
final_best27_backward <- regsubsets(Umsatz ~ ., data = df_lm_train_WG1, nvmax = 39, method = "backward")
coef(final_best27_backward, 27)
```
...
Für das 27-Variablenmodell der best subset selection wird nun ein Regressionsmodell erstellt:
```{r}
lm_WG1_27_backward_train <- lm(Umsatz ~ KielerWoche + Donnerstag + Freitag + Montag + Samstag + Sonntag + Dezember + Februar + Januar + Maerz + November + Oktober + September + SommerferienSH + SommerferienNRW + SommerferienNDS + SommerferienHE + Feiertag + Ostern + ChristiHimmelfahrt + Pfingsten + TDE +	Ostern_ext + ChristiHimmelfahrt_ext + Pfingsten_ext + Herbst + Sommer, data = df_lm_train_WG1_27)

library(broom)
glance(lm_WG1_27_backward_train)
```

```{r}
lm_WG1_27_backward_predict <- predict(lm_WG1_27_backward_train, newdata = df_lm_test_WG1_27)
```

Als Ergebnis wird ein Vektor erzeugt, der für jede Beobachtung des Test-Samples den geschätzten (vorhergesagten) Umsatzwert speichert. Die Gütewerte lassen sich mit `caret::postResample` ausgeben:
```{r}
caret::postResample(pred = lm_WG1_27_backward_predict, obs = df_lm_test_WG1$Umsatz)
```

Diese Vorhersage-Ergebnisse werden den Test-Daten hinzugefügt und an die gemeinsame Übersichtstabelle für die Gütekennzahlen `lm_vgl_kennz` angefügt, die nach und nach angereichert wird mit den Ergebnissen der anderen Modelle je Warengruppe:
```{r}
# Hinzufügen der Ergebnisse
df_lm_test_WG1_27 <- df_lm_test_WG1_27 %>%
  mutate(predicted = lm_WG1_27_backward_predict)
...
# füge das Modell hinzu
temp <- temp %>% mutate(Modell = "best27_backward_WG1")
...
**Teilmengenauswahl für das 30-Variablen-Modell gemäß backward stepwise**
Es erfolgt ein weiterer Vergleich mit dem 30 Variablen-Modell der best subset selection:

Die 30 Variablen gemäß best subset selection sind die folgenden:
...
```{r}
# Dummyfizierung der Variablen im Trainigsdatensatz:
df_lm_train_WG1_30 <- df_lm_train_WG1 %>%
  mutate(Montag=as.integer(df_lm_train_WG1$Wochentag_c=="Montag")) %>%
  mutate(Dienstag=as.integer(df_lm_train_WG1$Wochentag_c=="Dienstag")) %>%
  mutate(Mittwoch=as.integer(df_lm_train_WG1$Wochentag_c=="Mittwoch")) %>%
  mutate(Donnerstag=as.integer(df_lm_train_WG1$Wochentag_c=="Donnerstag")) %>%
...
df_lm_test_WG1_30 <- df_lm_test_WG1 %>%
  mutate(Montag=as.integer(df_lm_test_WG1$Wochentag_c=="Montag")) %>%
  mutate(Dienstag=as.integer(df_lm_test_WG1$Wochentag_c=="Dienstag")) %>%
  mutate(Mittwoch=as.integer(df_lm_test_WG1$Wochentag_c=="Mittwoch")) %>%
  mutate(Donnerstag=as.integer(df_lm_test_WG1$Wochentag_c=="Donnerstag")) %>%
...
Für das 30-Variablenmodell der best subset selection wird nun ein Regressionsmodell erstellt:

```{r}
lm_WG1_30_bss_train <- lm(Umsatz ~ Datum + Jahr + KielerWoche + Bewoelkung + Temperatur + Donnerstag + Freitag + Mittwoch + Montag + Samstag + Sonntag + Februar + Januar + Maerz + Oktober + September + SommerferienSH + SommerferienNRW + SommerferienNDS + SommerferienHE + Feiertag + Ostern + ChristiHimmelfahrt + Pfingsten + TDE + Ostern_ext + ChristiHimmelfahrt_ext + Pfingsten_ext + Herbst + Sommer, data = df_lm_train_WG1_30)

library(broom)
glance(lm_WG1_30_bss_train)
```

Dann wird (die Modellgüte) anhand der *Test*-Stichprobe getestet:

```{r}
lm_WG1_30_bss_predict <- predict(lm_WG1_30_bss_train, newdata = df_lm_test_WG1_30)
```

Als Ergebnis wird ein Vektor erzeugt, der für jede Beobachtung des Test-Samples den geschätzten (vorhergesagten) Umsatzwert speichert. Die Gütewerte lassen sich mit `caret::postResample` ausgeben:
```{r}
caret::postResample(pred = lm_WG1_30_bss_predict, obs = df_lm_test_WG1_30$Umsatz)
```

Diese Vorhersage-Ergebnisse werden den Test-Daten hinzugefügt und sodann eine gemeinsame Übersichtstabelle für die Gütekennzahlen angelegt `lm_vgl_kennz`, die nach und nach angereichert wird mit den Ergebnissen der anderen Modelle je Warengruppe:
```{r}
# Hinzufügen der Ergebnisse
df_lm_test_WG1_30 <- df_lm_test_WG1_30 %>%
  mutate(predicted = lm_WG1_30_bss_predict)
...
# Prognosegüte: mittlere Abweichung, mittlere absolute / relative Abweichung, mittlere quadratische Abweichung
temp <- df_lm_test_WG1_30 %>%
  group_by() %>%
  summarise(Anzahl=n(), Umsatz_ges = round(sum(Umsatz)), Umsatz_mittel = round(sum(Umsatz)/n()), MAE = round(mean(Abweichung_abs)), MPE = round(mean(Abweichung_rel)*100), MAPE = round(mean(Abweichung_rel_abs)*100), WAPE = round(sum(Abweichung_rel_abs_mult_umsatz)/sum(Umsatz)*100), MSE = round(mean(Abweichung_quad)), RMSE = round(sqrt(mean(Abweichung_quad))), rRMSE = round(RMSE/Umsatz_mittel*100))

# füge das Modell hinzu
temp <- temp %>% mutate(Modell = "best30_bss_WG1")
...
```{r}
best_subset_WG2 <- regsubsets(Umsatz ~ ., df_lm_train_WG2, nvmax = 39)
```
...
Für ein Modell mit einer Variablen kann beobachtet werden, dass die erzeugte Dummy-Variable `Sommer` ein Sternchen hat, was signalisiert, dass ein Regressionsmodell mit `Umsatz` ~ `Sommer` das beste Einzelvariablenmodell ist.  Das beste 2-Variablen-Modell ist `Umsatz` ~ `Sommer + Wochentag_cSonntag`. Das beste 3-Variablen-Modell ist `Umsatz` ~ `Sommer + Wochentag_cSonntag`. Und so weiter.
...
```{r}
forward <- regsubsets(Umsatz ~ ., df_lm_train_WG2, nvmax = 39, method = "forward")
```
...
```{r}
backward <- regsubsets(Umsatz ~ ., df_lm_train_WG2, nvmax = 39, method = "backward")
```
...
Es ist erkennbar, dass die Ergebnisse leicht unterschiedliche Modelle identifizieren, die als die besten angesehen werden. Die ajustierte $R^2$-Statistik legt nahe, dass das 34-Variablen-Modell bevorzugt wird, die $BIC$-Statistik schlägt das 22-Variablenmodell vor und der $C_p$ das 29-Variablen-Modell vor.
...
Wenn man das optimale $C_p$ für vorwärts und rückwärts schrittweise bewertet, ist erkennbar, dass gemäß der Vorwärts-Methode ein 29-Variablen-Modell die $C_p$-Statistik minimiert. Die Rückwärtsmethode schlägt ein 32-Variablen-Modell vor.

Wenn wir die Koeffizienten dieser Modelle bewerten, ergibt sich bzgl. der Zusammensetzung der Prädikatoren folgendes Bild: 
```{r}
# 22-Variablen-Modell
coef(best_subset_WG2, 22)
```
...
Beim 29-Variablen-Modell werden folgende Variablen gegenüber dem 22-V-M ergänzt:
...
```{r}
# 34-Variablen-Modell
coef(best_subset_WG2, 34)
```

Der Unterschied zwischen Prädiktoren im 29- und 34-Variablen-Modell besteht jedoch nicht in einer simplen Erweiterung des 34-Variablen-Modells. Vielmehr werden manche Variablen des 29-V-Modells nicht (mehr) aufgenommen, dafür andere inzugefügt. Einen Überblick über die Veränderungen ist der nachfolgenden Tabelle zu entnehmen:
...
Es ist erkennbar, dass das 30-Variablen-Modell, das durch den besten Teilmengenansatz erzeugt wird, den niedrigsten Test-MSE erzeugt. 
Auch ein 29- sowie 36-Variablen-Modell scheinen vergleichweichsweise gut zu performen.

Wir können jetzt die beste Teilmengenauswahl für den gesamten Datensatz durchführen, um zum einen das 30-Variablen-Modell zu erhalten.
Dieses Modell wird mit dem 19-und dem 29-Variablen-Modell verglichen.

**Teilmengenauswahl für das 30-Variablen-Modell**

```{r}
final_best_WG2_30 <- regsubsets(Umsatz ~ ., data = df_lm_train_WG2, nvmax = 36)
coef(final_best_WG2_30, 30)
```
...
glance(lm_WG2_30_train)
```

Dann wird (die Modellgüte) anhand der *Test*-Stichprobe getestet:

```{r}
lm_WG2_30_predict <- predict(lm_WG2_30_train, newdata = df_lm_test_WG2_30)
```

Als Ergebnis wird ein Vektor erzeugt, der für jede Beobachtung des Test-Samples den geschätzten (vorhergesagten) Umsatzwert speichert. Die Gütewerte lassen sich mit `caret::postResample` ausgeben:
```{r}
caret::postResample(pred = lm_WG2_30_predict, obs = df_lm_test_WG2_30$Umsatz)
```
...
Für das 29-Variablenmodell wird nun ein Regressionsmodell erstellt:
...
```{r}
lm_WG2_29_predict <- predict(lm_WG2_29_train, newdata = df_lm_test_WG2_29)
```

Als Ergebnis wird ein Vektor erzeugt, der für jede Beobachtung des Test-Samples den geschätzten (vorhergesagten) Umsatzwert speichert. Die Gütewerte lassen sich mit `caret::postResample` ausgeben:
```{r}
caret::postResample(pred = lm_WG2_29_predict, obs = df_lm_test_WG2_29$Umsatz)
```
...
```{r}
lm_WG2_19_predict <- predict(lm_WG2_19_train, newdata = df_lm_test_WG2_19)
```

Als Ergebnis wird ein Vektor erzeugt, der für jede Beobachtung des Test-Samples den geschätzten (vorhergesagten) Umsatzwert speichert. Die Gütewerte lassen sich mit `caret::postResample` ausgeben:
```{r}
caret::postResample(pred = lm_WG2_19_predict, obs = df_lm_test_WG2_19$Umsatz)
```
...
df_lm_test_WG4 <- df_lm_test_WG5 %>% dplyr::select(-Warengruppe)
```
...
```{r}
best_subset_WG5 <- regsubsets(Umsatz ~ ., df_lm_train_WG5, nvmax = 36)
```
...
# 26-Variablen-Modell
coef(best_subset_WG2, 26)

# 27-Variablen-Modell
coef(best_subset_WG2, 27)

# 28-Variablen-Modell
coef(best_subset_WG2, 28)

# 31-Variablen-Modell
coef(best_subset_WG2, 31)
```
...
Es ist erkennbar, dass das 16-Variablen-Modell, das durch den besten Teilmengenansatz erzeugt wird, den niedrigsten Test-MSE erzeugt. 
Auch ein 27/28- sowie 10-Variablen-Modell scheinen vergleichweichsweise gut zu performen.

Wir können jetzt die beste Teilmengenauswahl für den gesamten Datensatz durchführen, um zum einen das 16-Variablen-Modell zu erhalten.
Dieses Modell wird mit dem 10-und dem 28-Variablen-Modell verglichen.

**Teilmengenauswahl für das 16-Variablen-Modell**

```{r}
final_best_WG5_16 <- regsubsets(Umsatz ~ ., data = df_lm_train_WG5, nvmax = 36)
coef(final_best_WG5_16, 16)
```
...
Für das 16-Variablenmodell wird nun ein Regressionsmodell erstellt:
...
glance(lm_WG5_16_train)
```

Dann wird (die Modellgüte) anhand der *Test*-Stichprobe getestet:

```{r}
lm_WG5_16_predict <- predict(lm_WG5_16_train, newdata = df_lm_test_WG5_16)
```

Als Ergebnis wird ein Vektor erzeugt, der für jede Beobachtung des Test-Samples den geschätzten (vorhergesagten) Umsatzwert speichert. Die Gütewerte lassen sich mit `caret::postResample` ausgeben:
```{r}
caret::postResample(pred = lm_WG5_16_predict, obs = df_lm_test_WG5_16$Umsatz)
```
...
Für das 10-Variablenmodell wird nun ein Regressionsmodell erstellt:
...
```{r}
lm_WG5_10_predict <- predict(lm_WG5_10_train, newdata = df_lm_test_WG5_10)
```

Als Ergebnis wird ein Vektor erzeugt, der für jede Beobachtung des Test-Samples den geschätzten (vorhergesagten) Umsatzwert speichert. Die Gütewerte lassen sich mit `caret::postResample` ausgeben:
```{r}
caret::postResample(pred = lm_WG5_10_predict, obs = df_lm_test_WG5_10$Umsatz)
```
...
glance(lm_WG5_28_train)
```

Dann wird (die Modellgüte) anhand der *Test*-Stichprobe getestet:

```{r}
lm_WG5_28_predict <- predict(lm_WG5_28_train, newdata = df_lm_test_WG5_28)
```

Als Ergebnis wird ein Vektor erzeugt, der für jede Beobachtung des Test-Samples den geschätzten (vorhergesagten) Umsatzwert speichert. Die Gütewerte lassen sich mit `caret::postResample` ausgeben:
```{r}
caret::postResample(pred = lm_WG5_28_predict, obs = df_lm_test_WG5_28$Umsatz)
```
...
Wir haben jetzt die Gütekennzahlen für die Warengruppen 1, 2 und 5 ermittelt und zwar für verschiedene Modelle. Innerhalb der Warengruppen unterscheiden sich die Modelle nur marginal. Die besten Modelle für die Warengruppen 1, 2 und 5 sind die Modelle mit 36, 30 bzw. 16 Variablen.

In der Praxis könnte man sich auch dazu entschließen, jeweils das einfachste Modell zu verwenden, also mit möglichst wenig Variablen. Für die Warengruppe 1 ist es das Modell mit 31 Variablen, für die Warengruppe 2 mit 19 Variablen und für die Warengruppe 5 mit nur 10 Variablen.

Dann fällt auf, dass die Schätzer für die Warengruppe 2 offenbar systematisch zu hoch sind, weil die mittlere relative Abweichung bei 9% liegt. Für die übrigen Warengruppen liegt der Wert näher an Null.

Und der mittlere gewichtete Absolutwert der relativen Abweichung (WAPE), den wir vorrangig als Güte-Kriterium im Auge haben, zeigt den niedrigsten Wert für Warengruppe 2, gefolgt von Warengruppe 5. Ähnliche Ergebnisse hatten wir auch mit dem besten naiven Modell erzielt: Dort konnten mit dem erweiterten gleitenden Durchschnitt der letzten 4 Wochen- bzw. Wochenendtage die besten Ergebnisse für die Warengruppen insgesamt erzielt werden und für die Warengruppe 2 lag der WAPE mit nur 11 sogar knapp unter dem Wert des besten linearen Modells (12).
...
Der gewichtete mittlere Absolutwert der relativen Abweichung (WAPE) zeigt für die Warengruppe 2 den niedrigsten Fehler. Wir hatten jedoch für das beste naive und lineare Schätzmodell einen deutlich niedrigeren Fehler gesehen für Warengruppe 2 (WAPE = 11 bzw 12). Und auch unser SVM-Modell lag für die Warengruppe in der Größenordnung.
...
Wir hatten vier verschiedene Verfahren angewendet: Naive Modelle ("naiv"), lineare Regressionsmodelle ("lm"), Support Vector Machines ("SVM") und Multilayer Perceptrons ("MLP").
...
- Für die linearen Regressionsmodelle hatten wir gesehen, dass die Modelle mit 36 (best36), 30 (best30) bzw. 16 (best16) die besten Schätzer für die Warengruppen 1, 2 bzw. 5 lieferten.
...
- MPE: Die mittlere relative Abweichung gibt uns ein Indiz dafür, ob unser Modell die Umsätze systematisch zu hoch oder zu niedrig schätzt. Wir wollen natürlich möglichst einen Mittelwert nahe Null erzielen. Falls ein Modell jedoch den Umsatz systematisch zu hoch oder zu niedrig schätzt und ansonsten hervoragende Güte-Kennzahlen aufweist, kann man die Schätzwerte mit einem Offset korrigieren um eben diese mittlere relative Abweichung.
- WAPE: Der gewichtete Mittelwert des Absolutwertes der relativen Abweichung ist für das wichtigste Bewertungskriterium, weil es die Prognosegüte insgesamt am besten misst. Dabei gilt: Je kleiner, desto besser.
- rRMSE: Der Mittelwert der Wurzel der quadratischen Abweichung - ins Verhältnis gesetzt zum mittleren Umsatz - liefert uns Anhaltspunkte, ob vermehrt größere Abweichungen zwischen geschätztem und tatsächlichem Umsatz vorliegen. Wir wollen also hier möglichst niedrige Werte finden.
...
Für die Warengruppe 1 liefert das lineare Modell den besten Schätzer und zwar in Sachen MPE, WAPE und rRMSE.
...
Für Warengruppe 3 gewinnt ebenfalls das naive Modell. (Hinweis: Für diese Warengruppe hatten wir kein lineares Modell erstellt.)
...
Für Warengruppe 4 gewinnt wieder das naive Modell. (Hinweis: Für diese Warengruppe hatten wir kein lineares Modell erstellt.)
...
Für Warengruppe 5 gewinnt das lineare Modell knapp vor dem naiven Modell.
...
Wir bilden nun den Ensemble-Schätzer als Mittelwert der besten Einzelschätzer. Und anschließend ermittelns wir die Gütekennzahlen. Die Ergebnisse für die Warengruppen sammeln wir in einer gemeinsamen Tabelle `ensemble_WG_vgl`. HINWEIS: Solange wir noch kein lineares Modell für die Warengruppen 3 und 4 haben, wird der Ensemble-Schätzer für diese beiden Warengruppen nur aus dem naiven Schätzer, dem SVM und dem MLP Modell gebildet.

```{r}
# Warengruppe 1
# bilde Ensemble-Umsatz
ensemble_WG1 <- ensemble_WG1 %>% 
  mutate(Umsatz_ensemble = (Umsatz_naiv + Umsatz_lm + Umsatz_SVM + Umsatz_MLP) / 4)
...
ensemble_WG3 <- ensemble_WG3 %>% 
  mutate(Umsatz_ensemble = (Umsatz_naiv + Umsatz_SVM + Umsatz_MLP) / 3)
...
Für die Warengruppe 1 liefert das lineare Modell immer noch den besten Schätzer, knapp vor dem Ensemble-Modell.
...
Für Warengruppe 2 konnten wir durch die Ensemble-Bildung den Schätzer noch verbessern.
...
Für Warengruppe 3 liefert nach wie vor das naive Modell die besten Schätzer, allerdings nur hauchdünn vor dem Ensemble-Modell. (Hinweis: Für diese Warengruppe hatten wir kein lineares Modell erstellt.)
...
Für Warengruppe 4 können wir mit dem Ensemble die Prognosegüte noch minimal verbessern. (Hinweis: Für diese Warengruppe hatten wir kein lineares Modell erstellt.)
...
Durch einfache Ensemble-Bildung als Mittelwert der besten Einzelschätzer können wir für einige Warengruppen die Prognosegüte noch verbessern.

Und insgesamt lassen sich die Umsätze für die Warengruppe 2 (= Brötchen) am treffsichersten voraussagen. Brötchen scheinen also eine "Konstante" im Leben vieler Menschen hierzulande zu sein.

# Management Summary

Fehlt noch! Und dann an den Anfang stellen!
