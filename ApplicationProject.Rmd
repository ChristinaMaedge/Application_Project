...
> Anmerkung: Der 05.05.2014 gibt Rätsel auf. Ist es realistisch, dass die Umsätze an einem (gewöhnlichen) Montag derart hoch waren? Da wir keine Erklärung dafür haben, gehen wir davon aus, dass es sich um einen Datenfehler handelt oder ein singuläres Ereignis.
...
Maßgeblich ist für uns in erster Linie der Datensatz "Beispieldaten", da wir abschließend an der Prognose der Umsätze je Warengruppe interessiert sind. Wenn für ein Datum keine Umsätze vorhanden sind, sind die Daten aus den anderen Datensätzen grundsätzlich erst einmal auch unbrauchbar. Andererseits erfordern gewisse Modelle eine vollständige, ununterbrochenen Zeitreihe. Demzufolge wird zunächst ein neuer Datensatz ein `left_join` für die Vereinigung verwendet:
...
## 3.2 Korrektur der Anzahl Nachkommastellen für einzelne Variablen
...
Zwei der 7 verbleibenden Variablen enthalten Ausreißer: `Umsatz` und `Windgeschwindigkeit`. Beim Umgang mit den vorhandenen Ausreißern muss differenziert werden, ob es sich um unerwartete / nicht prognostizierbare Ausreißer handelt (Bsp. Mo, 05.05.2014) oder ob diese in gewisser Hinsicht planbar sind, weil sie erwartbar sind, da sie in allen Jahren gleichermaßen zu beobachten sind (z. B. Silvester). 
Nicht prognostizierbare und nicht erklärbare Ausreißer werden eliminiert. Prognostizierbare Ausreißer werden im weiteren Verlauf kodiert, d. h. es werden Variablen für diese planbaren Ausreißer angelegt.

Was die vorliegenden Daten anbelangt, sind einzig die Umsätze am Montag, 05.05.2014 nicht zu erklären. Dieser Tag ist der umsatzstärkste Tag im gesamten Zeitverlauf. Zudem ist es der einzige Tag im gesamten Datensatz, bei dem es Ausreißer für drei Warengruppen gibt (ansonsten beschränken sich die Ausreißer weitestgehend auf Warengruppe 2, vereinzelt gibt es Tage, an denen auch Warengruppe 5 Ausreißer aufweist, z. B. an Silvester). Da es sich hierbei um einen "gewöhnlichen" Montag handelt, erscheinen die Umsatzzahlen nicht plausibel und das Datum wird für alle Warengruppen eliminert. Im weiteren Verlauf werden bei der Erstellung der Analysdatensätze ggf. geeignete Imputationen für dieses Datum vorgenommen (z. B. Werte der Vorwoche, Mittelwert der vergangenen vier Montag o. Ä.).
...
## 3.6 Ergänzung um die Variablen Wochentag, Monat und Jahr
...
df <- df %>% mutate(Monat = month(Datum))
...
df <- df %>% mutate(Monat_c = recode(Monat, "1" = "Januar", "2" = "Februar", "3" = "März", "4" = "April", "5" = "Mai", "6" = "Juni", "7" = "Juli", "8" = "August", "9" = "September", "10" = "Oktober", "11" = "November", "12" = "Dezember"))
```
...
## 3.7 Ergänzung um Sommerferienvariablen
...
## 3.8 Ergänzung um Feiertagsvariablen
...
## 3.9 Ergänzung um Variable Jahreszeit
...
df <- left_join(df, Fruehling, by = "Datum")
...
df <- left_join(df, Sommer, by = "Datum")
...
df <- left_join(df, Herbst, by = "Datum")
...
df <- left_join(df, Winter, by = "Datum")
...
## 3.10 Rohdaten, Trainingsdaten, Testdaten

### Rohdatensatz

Ergebnis der vorangegegangenen Operationen ist der Rohdatensatz `df`. 

Dieser Rohdatensetz kann für einzelne Analysen ohne Weiteres verwendet werden, z. B. für eine Regressionsanalyse. Möchte man Vorhersagen auf Basis der Vorwochenwerte durchführen, z. B. die Umsatz-Prognose für den aktuellen Montag auf Basis des vorangegangenen Montags durchführen, könnten Probleme auftreten, da der Vorwochenwert aufgrund der unterbrochenenen Zeitreihe ggf. nicht verfügbar ist. 

Bsp.: Der Umsatz für den 02.01.2012 soll auf Basis der Vorwochenwerte prognostiziert werden. Der Vorwochenwert würde streng genommen der 26.12. sein, der 2. Weihnachtsfeiertag. Dieser Wert ist jedoch grundsätzlich fehlend, da die Filiale am 26.12. immer geschlossen hat. 

Führt man in R auf Basis der bestehenden Daten einen Prognose auf Basis der Vorwochenwerte durch, wird die Prognose für den 03.01. auf Basis des 23.12. vollzogen, für den 04.01. auf Basis von Heiligabend, ohne dass eine Fehelermeldung erzeugt wird.

Sinn und Zweck kann es allerdings nicht sein, dass die Prognosen eines Umsatzes am 03.01. sich an den Umstätzen des 24.12. orientieren. Folglich bedarf es eines weiteren Datensatzes, der eine vollständige Zeitreihe enthält. Wie die aufgrund der vorliegenden Datenbasis entstehenden unvollständigen Zeilen mit sinnvollen Werten aufzufüllen sind, ist noch / mit Blick auf die Anwendung verschiedener Modelle zu klären.

### Rohdaten mit vollständiger Zeitreihe
...
## Die Variablen Monat u. Jahr müssen nun für die vollständige Zeitreihe neu ermittelt werden
df_voll <- df_voll %>% mutate(Jahr = year(Datum))
df_voll <- df_voll %>% mutate(Monat = month(Datum))

df_voll <- df_voll %>%
  arrange(Warengruppe) # alle fehlenden Zeilen werden ans Ende des Datensatzes verschoben

# df_voll %>% filter(is.na(Umsatz))
```

### Trainings- und Testdaten

Wir verwenden den Zeitraum 2014 bis 2017 als Trainingsdaten. Die Daten des Jahres 2018 dienen als Testdaten. Dafür werden weitere Datensätze erstellt. Die Datensätze `df_train` und `df_test` basieren auf den Rohdaten `df`. Wohingegen die Datensätze `df_train_voll` und `df_test_voll` auf den Rohdaten mit vollständiger Zeitreihe basieren (`df_voll`).

```{r}
df_train <- df %>% filter(Jahr >= 2014 & Jahr <= 2017)
df_test <- df %>% filter(Jahr == 2018)
df_train_voll <- df_voll %>% filter(Jahr >= 2014 & Jahr <= 2017)
df_test_voll <- df_voll %>% filter(Jahr == 2018)
```
...
Untersuche den Umsatz je Wochentag und/oder Warengruppe in den Daten. Als Basis genügt dafür der Rohdatensatz `df`.

```{r}
df %>% group_by(Warengruppe) %>% summarise(Umsatz_sum = sum(Umsatz))
df %>% group_by(Wochentag_c) %>% summarise(Umsatz_sum = sum(Umsatz))
df %>% group_by(Warengruppe, Wochentag_c) %>% summarise(Umsatz_sum = sum(Umsatz))
df %>% group_by(Warengruppe, Wochentag) %>% summarise(Umsatz_sum = sum(Umsatz))
df %>% group_by(Warengruppe, Wochenende) %>% summarise(Umsatz_sum = sum(Umsatz))
```
...
In einem weiteren Schritt werden die Umsätze je Warengruppe und Monat untersucht, um eine differenziertere Veretilung der Umsätze im Jahresverlauf zu erhalten:


```{r}
df %>% group_by(Warengruppe, Monat) %>% summarise(Umsatz_sum = sum(Umsatz))

df %>% group_by(Warengruppe, Monat_c) %>% summarise(Umsatz_sum = sum(Umsatz)) %>% arrange(Warengruppe, desc(Umsatz_sum))
```
...
+ Tendenziell sind die Umsätze in den Wintermonaten (Dezember, Januar, Februar) sowie im November am schwächsten (auch ier mit leichten Abweichungen bei Warengruppe 4 und 5). Die Warengruppen Konditorei und Kuchen unterscheiden sich hier, mal mehr mal weniger, von den ersten drei Warengruppen.
...
* Die Umsätze im Jahr 2014 sind insgesamt offenbar parallel verschoben und systematisch höher jeden Monat. Der einzige Monat, für den das nicht zutrifft, ist der Mai: Hier hatten wir den 5.5.2014 als umsatzstärksten Tag des gesamten Zeitraums als potentiellen Datenfehler entfernt. 
...
* Insgesamt ist eine ähnliches Muster wie bei der Auswertung über alle Warengruppen erkennbar. Wieder haben die Variablen
  + Bewölkung
  + sämtliche betrachteten Sommerferien-Variablen
  + Silverster
  + Sommer
  einen Einfluss zu haben. Hinzu kommen der Wochentag und die erweiterte Osternvariable.
...
* Vom Muster her ergibt sich ein ähnliches Bild. Die Variablen
  + Bewölkung, 
  + Monat
  + Wochenende,
  ...
  * Weniger stark als bei Warengruppe 1 wirken die erweiterte Ostervariable und Silvester.
...
* Die Warengruppe unterscheidet sich abermals von den vorher betrachteten. Es sind insgesamt nur sehr schwache Effekte spürbar. 
+ Allenfalls die Variabeln Monat, Feiertag und Winter könnten einen spürbaren, wenn auch marginalen Einfluss haben.
...
# Prognose der Umsätze aufhand des Vorwochenwertes
```{r}
#df <- df %>% mutate(Umsatz_lag = lag(Umsatz, n=7), order_by = Warengruppe)

df <- df %>% group_by(Warengruppe) %>% mutate(Umsatz_lagged = lag(Umsatz, n=7, order_by = Datum))

df <- df %>% group_by(Warengruppe) %>% mutate(Datum_lagged= lag(Datum, n=7, order_by = Datum))
```
...
Im Rahmen einer linearen Regression ist es entscheidend, dass zwischen den Variablen, die in das Regressionsmodell einbezogen werden