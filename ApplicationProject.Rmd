...
Die erste Beobachtung ist, einige Daten tauchen mehrfach auftauchen. Besonders auffällig ist - wie schon bekannt - Silvester. Und dann fällt noch der 29.03.2018 auf.

Bemerkung: Für fast alle Daten ist die Prognose entweder konsequent zu hoch oder konsequent zu tief. Das Datum 07.01.2018 ist das einzige Datum, für das je zwei Schätzungen stark zu hoch bzw. stark zu niedrieg waren. Es handelt sich um den Tag eine Woche nach Silvester und es sind zwei verschiedene Warengruppen, wie wir gleich sehen werden...

wir nehmen zunächst die Warengruppe und das Modell hinzu:

```{r}
prog_naiv_vgl_top %>%
  group_by(Datum, Warengruppe, Modell, Prognose) %>%
  summarise(Anzahl = n()) %>%
  arrange(desc(Anzahl))
```
...
- Vergleichstabelle analysieren: Was steckt dahinter? Feiertage? Ferien? Wetter?
